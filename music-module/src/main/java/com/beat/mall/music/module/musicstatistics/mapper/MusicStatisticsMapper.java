package com.beat.mall.music.module.musicstatistics.mapper;
...
public interface MusicStatisticsMapper {
    ...
    @Select("SELECT music_count FROM music_statistics " +
            "WHERE is_deleted=0 AND year=#{year} AND month=#{month} AND day<=#{day} " +
     ...
     */
    @Select("SELECT music_count FROM music_statistics " +
            "WHERE is_deleted=0 AND year=#{year} AND month=#{month} " +
            "ORDER BY day DESC LIMIT 1")
     ...
     */
    @Select("SELECT music_count FROM music_statistics " +
            "WHERE is_deleted=0 AND year=#{year} " +
            "ORDER BY month DESC, day DESC LIMIT 1")
    Integer getByYear(@Param("year") String year);

    /**
     * 写入统计记录：存在则更新，不存在则插入（依赖 uk_date 唯一索引）
     * 冲突时仅更新业务字段与时间戳，不修改 is_deleted，避免复活已软删记录
     */
    @Insert("INSERT INTO music_statistics(year,month,day,music_count,create_time,update_time,is_deleted) " +
            "VALUES(#{year},#{month},#{day},#{musicCount},#{createTime},#{updateTime},#{isDeleted}) " +
            "ON DUPLICATE KEY UPDATE music_count=VALUES(music_count), create_time=VALUES(create_time), update_time=VALUES(update_time)")
